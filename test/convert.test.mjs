...
test('the grammar is read, the markup is not', () => {
  ...
  assert.deepEqual(model.sections.map((s) => s.type),
    ['hero', 'trustbar', 'split', 'cards', 'numbered', 'reviews', 'split', 'location', 'cta', 'footer']);
  assert.equal(model.sections[3].lists.items.length, 6);
  assert.equal(model.sections[9].lists.columns[1].lists.links.length, 5);
});
